...
def nor_adj(edge_idx, n):
    adj = sp.coo_matrix((np.ones(len(edge_idx[0])), (edge_idx[0], edge_idx[1])),  # 构建边的邻接矩阵,n表示矩阵维数
                        shape=(n, n),
                        dtype=np.float32)
    adj = adj + adj.T.multiply(adj.T > adj) - adj.multiply(adj.T > adj)  # build symmetric adjacency matrix，计算转置矩阵。将有向图转成无向图
