...
class Data():
    ...
    def __init__(self, graph,x, labels):
        ...
        # 训练，验证，测试的样本
        # idx_train = range(140)
        # idx_val = range(300, 500)
        # idx_test = range(500,1500)
        idx_train = range(int(labels.shape[0] * 0.9))
        ...
        incidence_matrix = normalize(nx.incidence_matrix(graph).A.astype(np.float16))
        self.incidence_matrix = torch.FloatTensor(incidence_matrix)
        self.line_graph = nx.line_graph(graph)
        self.embedding_edge_attr = self.embedding(self.line_graph,embedding_size = 64)
    ...
    def normalize(self,mx):
        ...
        r_mat_inv = sp.diags(r_inv).astype(np.float16)  # 构造对角戏矩阵
        mx = r_mat_inv.dot(mx).astype(np.float16)  # 构造D-1*A，非对称方式，简化方式
        return mx
...
def load_data(path="../data/cora/", dataset="cora"):  # cora citeseer
