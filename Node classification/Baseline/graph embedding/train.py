...
cora = graph
cora.remove_nodes_from(list(nx.isolates(cora)))  # 删除孤立顶点
# embedding model
...
node2vec_embed_model = Node2Vec(cora,10,80,p=1,q=0.5)
node2vec_embed_model.train()
node2vec_feature_embedding = node2vec_embed_model.get_embeddings()

line_X = np.array([line_feature_embedding[i] for i in list(cora.nodes)])
deepwalk_X = np.array([deepwalk_feature_embedding[i] for i in list(cora.nodes)])
node2vec_X = np.array([node2vec_feature_embedding[i] for i in list(cora.nodes)])
