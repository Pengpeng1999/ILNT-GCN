...
def load_citeseer(path="../data/citeseer/", dataset="citeseer"):
    ...
    # build graph
    idx = np.array(idx_features_labels[:, 0], dtype=np.dtype(str))  # 节点
    idx_map = {j: i for i, j in enumerate(idx)}   # 构建节点的索引字典
    edges_unordered = np.genfromtxt("{}{}.cites".format(path, dataset),  # 导入edge的数据
                                    dtype=np.dtype(str))
    # 
    ...
    for i in edges_unordered:
        if i[0] in idx and i[1] in idx:
            for j in i:
                edges.append(idx_map[j])
    edges = np.array(edges,
                     dtype=np.float32).reshape((int(len(edges)/2),2))    # 将之前的转换成字典编号后的边
    adj = sp.coo_matrix((np.ones(edges.shape[0]), (edges[:, 0], edges[:, 1])),  # 构建边的邻接矩阵
                        shape=(labels.shape[0], labels.shape[0]),
                        dtype=np.float32)
    ...
    for i in edges_unordered:
        if i[0] in idx and i[1] in idx:
            for j in i:
                edges.append(idx_map[j])
    edges = np.array(edges,
                     dtype=np.float32).reshape((int(len(edges)/2),2))
    adj = sp.coo_matrix((np.ones(edges.shape[0]), (edges[:, 0], edges[:, 1])),  # 构建边的邻接矩阵
                        shape=(labels.shape[0], labels.shape[0]),
                        dtype=np.float32)
